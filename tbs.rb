...
class Matrix
  ...
  def all_positions #return a list of cordinates pairs that exist in the matrix
    (0...@rows).collect do |y|
      (0...@cols).collect do |x|
        [x, y]
      end
...
class Action
  ...
  def self.range(unit); end
  def self.target?(unit, other); unit.enemy?(other); end

  # Defalt Aciton generator assumes action is something you do
  # to the enemy standing next to you. This behavior will be
  ...
  def self.generate(unit, game)
    ...
    return targetsi.collect{ |x, y| self.new(unit, game, x, y) }
  end
end
